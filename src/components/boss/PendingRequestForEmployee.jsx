...
import { useGetPendingEmployeeRequests } from "../../hooks/useGetPendingEmployeeRequests";
import { approveVacationEmployeeAPI, refuseVacationEmployeeAPI } from "../../services/api";
...
export const PendingRequestForEmployee = () => {
    const { pendingRequests, loading, error } = useGetPendingEmployeeRequests();
    ...
    const handleApprove = async (requestId) => {
        try {
            const response = await approveVacationEmployeeAPI(requestId, token);
    ...
    const handleReject = async (requestId) => {
        try {
            const response = await refuseVacationEmployeeAPI(requestId, token);
            toast.success(response.message);  // Mostrar alerta de éxito con react-hot-toast
            window.location.reload();  // Recargar para actualizar las solicitudes
        } catch (e) {
    ...
    return (
        <div className="p-4">
            ...
            <h3 className="text-xl font-bold mb-4 text-center">Solicitudes Pendientes de los Empleados</h3>
